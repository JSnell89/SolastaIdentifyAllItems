...
namespace SolastaIdentifyAllItems
{
    public class Main
    {
        ...
        {
            DirectoryInfo directoryInfo = new DirectoryInfo($@"{UnityModManager.modsPath}/SolastaIdentifyAllItems");
            FileInfo[] files = directoryInfo.GetFiles($"Translations-??.txt");

            foreach (var file in files)
            {
                var filename = $@"{UnityModManager.modsPath}/SolastaIdentifyAllItems/{file.Name}";
